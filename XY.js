...
//alert(ab);
	var cd=localStorage.getItem('varT');
//alert(cd);


    document.getElementById("target").value=ab;
    document.getElementById("temptext").value=cd;
    $("#edit_lines").empty();
    let values=document.getElementById("temptext").value.trim().split("\n");
    values.forEach((line)=>{
        console.log(line);
        let params=line.split(" ");
        let widthX=params[2].split(";");
        let heightY=params[3].split(";");
        if(!params[6]||!params[6].includes('#'))params[6]=getRandomColor();
        let last_line=addLineEditor(widthX[1]-widthX[0], heightY[0]-heightY[1], widthX[0], heightY[1], params[6]);
        last_line.find(".r_columnX option[value="+params[1]+"]")[0].selected=true;
        last_line.find(".r_columnY option[value="+params[0]+"]")[0].selected=true;
        last_line.find(".r_width").change();
    });
...
function DTMA() {
    ...
    for (var i = 1; i < textArray.length; i++) {
        ...
        if (newNumber1 > nmax && newNumber1 < 1000000000 && i < 1001) {
            nmax = newNumber1;
            Tmax = i;
        }
        if ((i % 100) == 0) {
            ...
            if (nakomax > nmax && nakomax < 1000000000 && nmax !== 0 && i < 1001) {
                nakomax = nmax;
                Tnako = Tmax;
            }
            nmax = 0;
...
function DTMM() {
    ...
    for (var k = 0; k < 10; k++) {
        ...
        var Dnak1 = Math.abs(nakomin[k]);
        var Dnak2 = Math.abs(nakomax[k]);
...
//
        var Dmirr = nakomin[k].toString() + ";" + nakomax[k].toString(); //mirror


        var Dmirr = Dnak1.toString() + ";" + Dnak2.toString(); //mirror
//////////alert(Dmirr);


        var ta45 = document.getElementById("che12").value;

        var ta444 = ta45.split(' ');

        var ta40 = ta444[0];
        var ta50 = ta444[1];

        if (ta40 == '2' && k == 0) Dmirr = Dnak1.toString() + ";" + Dnak2.toString(); //mirror
        if (ta50 == '2' && k == 2) Dmirr = Dnak1.toString() + ";" + Dnak2.toString(); //mirror
...
//////////////alert(Dnak1.toString()+";"+Dnak2.toString());

        if (ta40 == '2' && k == 0) nakot = DeltaRange(Dnak1.toString() + ";" + Dnak2.toString(), 10);
        if (ta50 == '2' && k == 2) nakot = DeltaRange(Dnak1.toString() + ";" + Dnak2.toString(), 10);
...
function CARE()             //Changed
{
    ...
    var textFrom = document.getElementById("textFrom").value || -10;
    ...
    if ($("#edit_lines").length > 0) {
        var column_amount = tts.length;
        var ab11 = "0 1 20;1100 810;10 2 2 red 1 1";

        $("#edit_lines").empty();

        addLineEditor(1100, 810, 20, 10, '#ff0000');

        for (var i = 1; i < column_amount; i++) {
            var randColor = getRandomColor();
            if ($("#edit_lines").length == 0) randColor = "black";
            ab11 += "\n0 " + (i + 1) + " 20;1100 810;10 2 2 " + randColor + " 0 0";
            addLineEditor(1100, 810, 20, 10, randColor);
        }
        document.getElementById('temptext').value = ab11;

        for (var i = 1; i <= column_amount; i++) {
            if ($("#edit_lines").length > 0) $(".plot_line:nth-of-type(" + (i) + ") .r_columnX option")[i].selected = true;
        }

        if ($("#edit_lines").length > 0) {
            $(".plot_line:first .r_columnX_type option[value=2]").attr("selected", true);
            $(".plot_line:first .r_columnY_type option[value=2]").attr("selected", true);
            $(".plot_line:not(:first) .r_columnX_type option[value=-1]").attr("selected", true);
            $(".plot_line:not(:first) .r_columnY_type option[value=-1]").attr("selected", true);
        }
    }
...
//alert(str5);
    ...
    if (delim == ",") {
        document.getElementById(textarea).value = str8;
    }
...
function loadDTMAtext() {
    ...
    del9();
    if ($("#edit_lines").length == 0) {
        var ab11 = "0 1 20;620 220;20 0 0 blue 1 1\n0 2 20;620 220;20 0 0 red 0 0\n0 3 20;620 420;220 0 2 navy 0 0\n0 4 20;620 420;220 0 2 brown 0 1";
        document.getElementById('temptext').value = ab11;
    }
...
///var tu=s.replace
///////////alert(s);
        var textArray = tf.split("\n");

        if ($("#edit_lines").length > 0) {
            var column_amount = textArray[0].split(" ").length - 1;
            var ab11 = "0 1 20;620 220;20 0 0 blue 1 1";
            $("#edit_lines").empty();

            addLineEditor(620, 220, 20, 20, '#0000ff');

            for (var i = 1; i < column_amount - 1; i++) {
                var randColor = getRandomColor();
                if ($("#edit_lines").length == 0) randColor = "black";
                if (i >= 2 && i < 4) {
                    ab11 += "\n0 " + (i + 1) + " 20;620 420;220 0 0 " + randColor + " 0 " + i % 2;
                    addLineEditor(620, 420, 20, 220, randColor);
                } else {
                    ab11 += "\n0 " + (i + 1) + " 20;620 220;20 0 0 " + randColor + " 0 0";
                    addLineEditor(620, 220, 20, 20, randColor);
                }
            }
            document.getElementById('temptext').value = ab11;
            ...
            if ($("#edit_lines").length > 0) {
                $(".plot_line:not(:first) .r_columnX_type option[value=-1]").attr("selected", true);
                $(".plot_line:not(:first) .r_columnY_type option[value=-1]").attr("selected", true);
                $(".plot_line:nth-of-type(4) .r_columnX_type").attr("selected", false);
                $(".plot_line:nth-of-type(4) .r_columnX_type option[value=0]").attr("selected", true);
            }
        }
...
///    textFrom2 = document.getElementById("inputTextToSave").value
        ...
        var column_amount = textArray[0].split("\t").length;
        var ab11 = "0 1 20;1100 800;10 0 0 red 1 1";

        $("#edit_lines").empty();

        addLineEditor(1100, 800, 20, 10, '#ff0000');

        for (var i = 1; i < column_amount - 1; i++) {
            var randColor = getRandomColor();
            if ($("#edit_lines").length == 0) randColor = "black";
            ab11 += "\n0 " + (i + 1) + " 20;1100 800;10 0 0 " + randColor + " 0 0";
            addLineEditor(1100, 800, 20, 10, randColor);
        }
        document.getElementById('temptext').value = ab11;
...
//////////////////////////////////////////////////alert('loadcsvtext');
    ...
    document.getElementById("ftype").value = "JM";


    del9();

    var ab11 = "0 1 20;1100 800;10 2 2 black 1 1\n0 2 20;1100 800;10 2 2 red 0 0\n0 6 20;1100 800;10 2 2 blue 0 0";
...
//
    var ab22 = document.getElementById('temptext').value;

    tb22 = ab22.split("\n");

//alert(tb22[1]);

    var tu22 = tb22[0].split(" ");

//alert(tu22[4]+" "+tu22[5]);

    document.getElementById("che12").value = tu22[4] + " " + tu22[5];
...
//var ni=xsmall.length;
    ...
    if (Math.abs(vmax) < 0.001) {
        smax = vmax.toExponential(1);
    }
...
function findval(x1) {
    ...
    for (var i = 1; i < textArray.length; i++) {
        ...
        if (temp > x1) {
            Tneo = temps[0];
            Tneo = Tneo.substr(8, 2) + "." + Tneo.substr(5, 2);
            break;
        }
...
///////////////////
        table_time(from1,to1);
        if (temp > from1 && temp < to1) {
            Tneo = Tneo + textArray[i] + '\n';
        }
